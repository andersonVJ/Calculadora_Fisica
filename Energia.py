    ...
    def calcular_trabajo(self): 
        ...
        # Convertir unidades
            ...
            # Calcular fuerza normal
                fuerza_normal = masa * 9.81
            
            # Calcular trabajo total
                trabajo = calcular_trabajo(fuerza, desplazamiento, angulo, coef_friccion, masa)
            else:
                masa = float(masa) if masa else 0  # Asegurar que masa tenga un valor
                fuerza_normal = masa * 9.81  # Calculamos la fuerza normal si solo se proporciona masa
                trabajo = calcular_trabajo(fuerza, desplazamiento, angulo)

        # Calcular incertidumbre
            incertidumbre_fuerza = 0.05 * fuerza  # 5% de incertidumbre en la fuerza
            incertidumbre_desplazamiento = 0.05 * desplazamiento  # 5% de incertidumbre en el desplazamiento
            incertidumbre_trabajo = abs(
            (incertidumbre_fuerza * desplazamiento * math.cos(math.radians(angulo))) +
            (fuerza * incertidumbre_desplazamiento * math.cos(math.radians(angulo)))
            )
        ...
        self.resultado_normal.config(text=f"Fuerza Normal: {fuerza_normal:.2f} N")

        # Pasos
        pasos = f"Pasos:\n"
        pasos += f"1. Convertir fuerza a N: {fuerza:.2f} N\n"
        pasos += f"2. Convertir desplazamiento a m: {desplazamiento:.2f} m\n"
        pasos += f"3. Calcular fuerza normal: {fuerza_normal:.2f} N\n"
        pasos += f"4. Calcular trabajo de la fuerza = F * d * cos(θ) = {fuerza:.2f} * {desplazamiento:.2f} * cos({angulo}°) = {fuerza * desplazamiento * math.cos(math.radians(angulo)):.2f} J\n"
        
        if coef_friccion and masa:
                pasos += f"5. Calcular trabajo de la fricción = -μ * m * g * d = -{coef_friccion:.2f} * {masa:.2f} * 9.8 * {desplazamiento:.2f} = {-coef_friccion * masa * 9.8 * desplazamiento:.2f} J\n"
                pasos += f"6. Calcular trabajo total = {trabajo:.2f} J\n"
        
                pasos += f"7. Convertir resultado a {self.resultado_trabajo_unidad.get()}: {trabajo_convertido:.2f} {self.resultado_trabajo_unidad.get()}\n"
                pasos += f"8. Incertidumbre en el trabajo: ±{incertidumbre_trabajo:.2f} J\n"
